//
//  IO.NonBlocking.Registration.Reply.Bridge.swift
//  swift-io
//
...
extension IO.NonBlocking.Registration.Reply {
    ...
    public final class Bridge: @unchecked Sendable {
        ...
        struct State {
            var replies: [IO.NonBlocking.Registration.Reply] = []
            var continuation: CheckedContinuation<IO.NonBlocking.Registration.Reply?, Never>?
...
extension IO.NonBlocking.Registration.Reply.Bridge {
    ...
    public func push(_ reply: IO.NonBlocking.Registration.Reply) {
        // Extract continuation inside lock, resume OUTSIDE lock
        let continuationToResume: CheckedContinuation<IO.NonBlocking.Registration.Reply?, Never>? = state.withLock { state in
    ...
    public func next() async -> IO.NonBlocking.Registration.Reply? {
        enum Action {
            case returnNil
            case returnReply(IO.NonBlocking.Registration.Reply)
            case suspend
        }
