...
extension IO.Event.Bridge {
    ...
    public func push(_ events: [IO.Event]) {
        // Extract continuation inside lock, resume OUTSIDE lock
        let continuationToResume: CheckedContinuation<[IO.Event]?, Never>? = state.withLock { state in
    ...
    public func next() async -> [IO.Event]? {
        enum Action {
            case returnNil
            case returnBatch([IO.Event])
            case suspend
        }
