...
extension IO.Completion.IOCP {
    ...
    static func submitStorage(
        ...
        case .nop, .wakeup:
            ...
            break
        case .accept:
            try submitAccept(handle, storage, state)
        case .connect:
            try submitConnect(handle, storage, state)
        default:
    ...
    static func poll(
        ...
        for i in 0..<Int(numRemoved) {
            ...
            if entry.lpOverlapped == nil {
                buffer.append(IO.Completion.Event(
    ...
    static func createWakeupChannel(
        ...
        return IO.Completion.Wakeup.Channel(
            wake: {
                ...
                _ = PostQueuedCompletionStatus(rawHandle, 0, 0, nil)
            },
...
extension IO.Completion.IOCP {
    ...
    private static func associateIfNeeded(
        ...
        let result = CreateIoCompletionPort(fileHandle, iocpHandle, 0, 0)
        if result == nil {
            throw .kernel(.platform(
                code: Int32(GetLastError()),
                message: "Failed to associate handle with IOCP"
            ))
        }
...
extension IO.Completion.IOCP {
    ...
    ///
    /// AcceptEx requires additional setup (accept socket creation, address buffers).
    /// This is a placeholder for Phase 2.5/3.
    private static func submitAccept(
        ...
        // AcceptEx requires:
        // 1. Create accept socket (WSASocket)
        // 2. Allocate address buffer (local + remote addresses)
        // 3. Get AcceptEx function pointer via WSAIoctl (WSAID_ACCEPTEX)
        // 4. Call AcceptEx
        //
        // Deferred to Phase 2.5/3 when socket support is added
        throw .capability(.unsupportedKind(.accept))
    }

    /// Submits a connect operation.
    ///
    /// ConnectEx requires additional setup (address, function pointer lookup).
    /// This is a placeholder for Phase 2.5/3.
    private static func submitConnect(
        ...
        // ConnectEx requires:
        // 1. Get ConnectEx function pointer via WSAIoctl (WSAID_CONNECTEX)
        // 2. Socket must be bound first
        // 3. Call ConnectEx with address
        //
        // Deferred to Phase 2.5/3 when socket support is added
        throw .capability(.unsupportedKind(.connect))
    }
}
