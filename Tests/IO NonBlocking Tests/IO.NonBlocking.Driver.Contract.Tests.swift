//
//  IO.NonBlocking.Driver.Contract.Tests.swift
//  swift-io
//
...
@Suite("IO.NonBlocking.Driver.Contract")
struct DriverContractTests {
    ...
    func registerCreatesMapping() throws {
        let controller = IO.NonBlocking.Fake.Controller()
        let driver = IO.NonBlocking.Fake.driver(controller: controller)
    ...
    func modifyUnregisteredThrows() throws {
        ...
        let fakeID = IO.NonBlocking.ID(raw: 999)

        #expect(throws: IO.NonBlocking.Error.notRegistered) {
    ...
    func pollDropsDeregisteredEvents() throws {
        ...
        controller.pushEvent(
            IO.NonBlocking.Event(id: id, interest: .read, flags: []),
            handle: handle
        )
        try driver.deregister(handle, id: id)

        // Poll should return 0 events (the event was for a deregistered ID)
        var buffer = [IO.NonBlocking.Event](repeating: .empty, count: 10)
    ...
    func pollReturnsMultipleEvents() throws {
        ...
        controller.pushEvents([
            IO.NonBlocking.Event(id: id1, interest: .read, flags: []),
            IO.NonBlocking.Event(id: id2, interest: .write, flags: [])
        ], handle: handle)
    ...
    func shutdownRejectsRegistrations() throws {
        ...
        #expect(throws: IO.NonBlocking.Error.self) {
...
extension IO.NonBlocking.Event {
    ...
    static var empty: IO.NonBlocking.Event {
        IO.NonBlocking.Event(id: IO.NonBlocking.ID(raw: 0), interest: [], flags: [])
    }
}
