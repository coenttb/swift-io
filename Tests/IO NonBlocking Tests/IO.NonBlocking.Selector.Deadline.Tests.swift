//
//  IO.NonBlocking.Selector.Deadline.Tests.swift
//  swift-io
//
...
@Suite("IO.NonBlocking.Selector.Deadline")
...
extension SelectorDeadlineTests {
    ...
    func armWithDeadlineTimesOut() async throws {
        ...
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 50)
    ...
    func eventBeatsTimeout() async throws {
        ...
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 5000)
    ...
    func tickWakesSelectorForDeadlineOnly() async throws {
        ...
        let start = IO.NonBlocking.Deadline.now
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 30)
        ...
        do {
            ...
            case .timeout:
                ...
                let elapsed = IO.NonBlocking.Deadline.now.nanoseconds - start.nanoseconds
...
extension SelectorDeadlineTests {
    ...
    func eventBumpsGeneration() async throws {
        ...
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 60_000)
...
extension SelectorDeadlineTests {
    ...
    func concurrentDeadlinesBothFire() async throws {
        ...
        let d1 = IO.NonBlocking.Deadline.after(milliseconds: 30)
        let d2 = IO.NonBlocking.Deadline.after(milliseconds: 60)

        let start = IO.NonBlocking.Deadline.now

        // Use armTwo - creates both waiters before either completes
        let (outcome1, outcome2) = await selector.armTwo(
            IO.NonBlocking.Arm.Request(token: reg1.token, interest: .read, deadline: d1),
            IO.NonBlocking.Arm.Request(token: reg2.token, interest: .read, deadline: d2)
        )
    ...
    func sequentialDeadlinesSmoke() async throws {
        ...
        // First deadline
        let reg1 = try await selector.register(pipe1.read, interest: .read)
        let deadline1 = IO.NonBlocking.Deadline.after(milliseconds: 30)
        ...
        // Second deadline
        let reg2 = try await selector.register(pipe2.read, interest: .read)
        let deadline2 = IO.NonBlocking.Deadline.after(milliseconds: 30)
...
extension SelectorDeadlineTests {
    ...
    func deadlineAfterCreatesFuture() {
        let now = IO.NonBlocking.Deadline.now
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 100)
    ...
    func expiredDeadlineHasExpiredTrue() async throws {
        let deadline = IO.NonBlocking.Deadline.after(milliseconds: 10)
    ...
    func deadlineNeverNeverExpires() {
        let deadline = IO.NonBlocking.Deadline.never
        #expect(!deadline.hasExpired)
...
extension SelectorDeadlineTests {
    ...
    func minHeapOrdersByDeadline() {
        typealias Entry = IO.NonBlocking.DeadlineScheduling.Entry
        typealias MinHeap = IO.NonBlocking.DeadlineScheduling.MinHeap

        var heap = MinHeap()

        // Insert in non-sorted order
        heap.push(Entry(deadline: 300, key: IO.NonBlocking.Selector.PermitKey(id: .init(raw: 1), interest: .read), generation: 1))
        heap.push(Entry(deadline: 100, key: IO.NonBlocking.Selector.PermitKey(id: .init(raw: 2), interest: .read), generation: 1))
        heap.push(Entry(deadline: 200, key: IO.NonBlocking.Selector.PermitKey(id: .init(raw: 3), interest: .read), generation: 1))
    ...
    func minHeapPeekDoesntRemove() {
        ...
        var heap = MinHeap()
        heap.push(Entry(deadline: 100, key: IO.NonBlocking.Selector.PermitKey(id: .init(raw: 1), interest: .read), generation: 1))
