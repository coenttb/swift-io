//
//  IO.NonBlocking.Channel.Tests.swift
//  swift-io
//
...
extension IO.NonBlocking.Channel.Test {
    @Suite struct BasicIO {}
}

extension IO.NonBlocking.Channel.Test.BasicIO {
...
extension IO.NonBlocking.Channel.Test.EOF {
    /// Helper to create a non-blocking pipe
    private static func makeNonBlockingPipe() throws -> (read: Int32, write: Int32) {
        var fds: (Int32, Int32) = (0, 0)
        let result = withUnsafeMutablePointer(to: &fds) { ptr in
...
extension IO.NonBlocking.Channel.Test.Close {
    /// Helper to create a non-blocking pipe
    private static func makeNonBlockingPipe() throws -> (read: Int32, write: Int32) {
        var fds: (Int32, Int32) = (0, 0)
        let result = withUnsafeMutablePointer(to: &fds) { ptr in
