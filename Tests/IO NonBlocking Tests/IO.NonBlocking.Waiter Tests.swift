//
//  IO.NonBlocking.Waiter Tests.swift
//  swift-io
//
...
extension IO.NonBlocking.Waiter {
    #TestSuites
}

// MARK: - Unit Tests

extension IO.NonBlocking.Waiter.Test.Unit {
    @Test("can create unarmed waiter")
    func canCreateUnarmed() async {
        let id = IO.NonBlocking.ID(raw: 42)
        let waiter = IO.NonBlocking.Waiter(id: id)
        #expect(waiter.id == id)
    ...
    func armBindsContinuation() async {
        await withCheckedContinuation { (continuation: CheckedContinuation<Result<IO.NonBlocking.Event, IO.NonBlocking.Failure>, Never>) in
            let waiter = IO.NonBlocking.Waiter(id: IO.NonBlocking.ID(raw: 1))
            #expect(!waiter.isArmed)
...
extension IO.NonBlocking.Waiter.Test.EdgeCase {
    ...
    func armTwiceReturnsFalse() async {
        ...
        let _: Result<IO.NonBlocking.Event, IO.NonBlocking.Failure> = await withCheckedContinuation { first in
        ...
        let _: Result<IO.NonBlocking.Event, IO.NonBlocking.Failure> = await withCheckedContinuation { second in
...
extension IO.NonBlocking.Waiter.Test {
    @Suite struct Invariants {}
}

extension IO.NonBlocking.Waiter.Test.Invariants {
    ...
    func typedResultContinuation() async {
        ...
        let result: Result<IO.NonBlocking.Event, IO.NonBlocking.Failure> = await withCheckedContinuation { continuation in
            let waiter = IO.NonBlocking.Waiter(id: IO.NonBlocking.ID(raw: 1))
            waiter.arm(continuation: continuation)
