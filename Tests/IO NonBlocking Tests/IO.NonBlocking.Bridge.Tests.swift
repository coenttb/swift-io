//
//  IO.NonBlocking.Bridge.Tests.swift
//  swift-io
//
...
extension IO.NonBlocking.Event.Bridge.Test.Unit {
    @Test("next returns nil after shutdown")
    func nextReturnsNilAfterShutdown() async {
        let bridge = IO.NonBlocking.Event.Bridge()
        bridge.shutdown()
...
extension IO.NonBlocking.Event.Bridge.Test.EdgeCase {
    @Test("multiple pushes queue correctly")
    func multiplePushesQueueCorrectly() async {
        let bridge = IO.NonBlocking.Event.Bridge()

        let event1 = IO.NonBlocking.Event(id: IO.NonBlocking.ID(raw: 1), interest: .read, flags: [])
        let event2 = IO.NonBlocking.Event(id: IO.NonBlocking.ID(raw: 2), interest: .write, flags: [])
...
extension IO.NonBlocking.Registration.Reply.Bridge {
    #TestSuites
}

extension IO.NonBlocking.Registration.Reply.Bridge.Test.Unit {
    @Test("next returns nil after shutdown")
    func nextReturnsNilAfterShutdown() async {
        let bridge = IO.NonBlocking.Registration.Reply.Bridge()
        bridge.shutdown()
        let result = await bridge.next()
        #expect(result == nil)
    }
    ...
    func pushAfterShutdownIsIgnored() async {
        ...
        let reply = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 1),
    ...
    func pushThenNextReturnsReply() async {
        let bridge = IO.NonBlocking.Registration.Reply.Bridge()
        let reply = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 42),
            result: .success(.registered(IO.NonBlocking.ID(raw: 100)))
        )
        bridge.push(reply)
    ...
    func nextThenPushResumesExactlyOnce() async {
        ...
        let reply = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 77),
...
extension IO.NonBlocking.Registration.Reply.Bridge.Test.EdgeCase {
    @Test("multiple pushes queue correctly")
    func multiplePushesQueueCorrectly() async {
        let bridge = IO.NonBlocking.Registration.Reply.Bridge()

        let reply1 = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 1),
            result: .success(.registered(IO.NonBlocking.ID(raw: 10)))
        )
        let reply2 = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 2),
            result: .success(.deregistered)
        )
    ...
    func errorRepliesDeliveredCorrectly() async {
        ...
        let reply = IO.NonBlocking.Registration.Reply(
            id: IO.NonBlocking.Registration.ReplyID(raw: 5),
