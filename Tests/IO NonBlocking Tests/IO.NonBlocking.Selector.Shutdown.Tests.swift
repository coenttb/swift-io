//
//  IO.NonBlocking.Selector.Shutdown.Tests.swift
//  swift-io
//
...
extension IO.NonBlocking.Selector.Test {
    @Suite struct Shutdown {}
}

extension IO.NonBlocking.Selector.Test.Shutdown {
    ...
    func shutdownRejectsNewRegistrations() async throws {
        ...
        do {
            _ = try await selector.register(0, interest: IO.NonBlocking.Interest.read)
            Issue.record("register should throw after shutdown")
        } catch let error {
            // `error` is typed as IO.NonBlocking.Failure, no `as` cast needed
            switch error {
        ...
        for iteration in 0..<10 {
            ...
            let registerTask = Task {
                do throws(IO.NonBlocking.Failure) {
                    _ = try await selector.register(readFD, interest: IO.NonBlocking.Interest.read)
                    return Result<Void, IO.NonBlocking.Failure>.success(())
                } catch {
                    // `error` is typed as IO.NonBlocking.Failure
                    return Result<Void, IO.NonBlocking.Failure>.failure(error)
                }
            }
...
extension IO.NonBlocking.Selector.Test.Invariants {
