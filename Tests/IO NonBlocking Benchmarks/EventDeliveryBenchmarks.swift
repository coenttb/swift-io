            ...
            let producerTask = Task {
                for i in 0..<10000 {
                    let event = IO.NonBlocking.Event(
                        id: IO.NonBlocking.ID(raw: UInt64(i)),
                        interest: .read,
                        flags: []
                    )
                    bridge.push([event])
            ...
            let producerTask = Task {
                for i in 0..<10000 {
                    let reply = IO.NonBlocking.Registration.Reply(
                        id: IO.NonBlocking.Registration.ReplyID(raw: UInt64(i)),
                        result: .success(.registered(IO.NonBlocking.ID(raw: UInt64(i))))
                    )
                    bridge.push(reply)
                }
                bridge.shutdown()
        ...
        func eventBridgeBatchedThroughput() async {
            ...
            let producerTask = Task {
                for batchIndex in 0..<1000 {
                    var batch: [IO.NonBlocking.Event] = []
                    for i in 0..<10 {
                        let event = IO.NonBlocking.Event(
                            id: IO.NonBlocking.ID(raw: UInt64(batchIndex * 10 + i)),
                            interest: .read,
                            flags: []
                        )
                        batch.append(event)
                ...
                for producerIndex in 0..<producerCount {
                    group.addTask {
                        for i in 0..<eventsPerProducer {
                            let event = IO.NonBlocking.Event(
                                id: IO.NonBlocking.ID(raw: UInt64(producerIndex * eventsPerProducer + i)),
                                interest: .read,
                                flags: []
                            )
                            bridge.push([event])
