//
//  IO.Blocking.Lane.Test Tests.swift
//  swift-io
//
//  Tests for the fault-tolerant test lane.
//
...
extension IO.Blocking.Lane.Test.Test.Unit {
    @Test("test lane has correct capabilities")
    func testLaneCapabilities() async {
        let test = IO.Blocking.Lane.test()
        #expect(test.lane.capabilities.executesOnDedicatedThreads == true)
        #expect(test.lane.capabilities.executionSemantics == .abandonOnExecutionTimeout)
        await test.lane.shutdown()
    }

    @Test("test lane executes simple operation")
    func testLaneExecutesSimpleOperation() async throws {
        let test = IO.Blocking.Lane.test()

        let result: Result<Int, Never> = try await test.lane.run(deadline: nil) { 42 }
        #expect(try result.get() == 42)

        await test.lane.shutdown()
    }

    @Test("test lane executes multiple operations")
    func testLaneExecutesMultipleOperations() async throws {
        let test = IO.Blocking.Lane.test(.init(workers: 2))

        for i in 0..<10 {
            let result: Result<Int, Never> = try await test.lane.run(deadline: nil) { i * 2 }
    ...
    func metricsReflectCompletedOperations() async throws {
        let test = IO.Blocking.Lane.test(.init(workers: 1))

        // Execute a few operations
        for _ in 0..<5 {
            let _: Result<Int, Never> = try await test.lane.run(deadline: nil) { 1 }
        }
...
extension IO.Blocking.Lane.Test.Test.EdgeCase {
    ...
    func operationTimeoutAbandonsWorker() async throws {
        ...
        do {
            let _: Result<Int, Never> = try await test.lane.run(deadline: nil) {
    ...
    func multipleTimeoutsSpawnMultipleReplacements() async throws {
        ...
        let result: Result<Int, Never> = try await test.lane.run(deadline: nil) { 100 }
    ...
    func concurrentOperationsCompleteCorrectly() async throws {
        ...
        await withTaskGroup(of: Int.self) { group in
            for i in 0..<100 {
                group.addTask {
                    do {
                        let result: Result<Int, Never> = try await test.lane.run(deadline: nil) { i }
